...
class mercury_widget_9 extends WP_Widget {

/*  Widget #9 Setup  */

	public function __construct() {
		parent::__construct(false, $name = esc_html__('#9 Homepage Block [Mercury]', 'spacethemes' ), array(
			'description' => esc_html__('#9 Widget for HOMEPAGE', 'spacethemes' )
		));
	}
				...
				<div class="space-news-9-item box-100 relative">
					<div class="space-news-9-item-ins case-15 relative">
						<div class="space-news-9-item-background box-100 relative">
							...
							<?php
							$widget_9_img = wp_get_attachment_image_src(get_post_thumbnail_id(), 'mercury-585-505');
							if ($widget_9_img) { ?>
								<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
									<img src="<?php echo esc_url($widget_9_img[0]); ?>" alt="<?php the_title_attribute(); ?>">
...
add_action( 'widgets_init', 'mercury_widget_9_register' );

function mercury_widget_9_register() {
	register_widget( 'mercury_widget_9' );
}